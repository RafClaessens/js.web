//69, 666, 420, 1992 <- EASTER EGG HUNT just for fun, suprising people is fun
...
function clickButton(event) {
  $(".btn").click(function(event) {
    ...
    if ($(this).hasClass("cyan")) {
      console.log(`clickButton-1. ${thingClicked} is in the cyan class!`)
...
function addSymbolToAnswer(string) {
  $('.negative').prop('disabled', true) //disable "+/-" when cyan button clicked
  var symbolString = string
  symbolString = replaceTimesAndDivides(symbolString)
  console.log('cyan-1. symbolString is: ', symbolString)
  var currentValue = getCurrentValue()
  var symbols = ["*", "/", "+", "-", "%"]
  console.log('cyan-2. globalAnswer is now: ', globalAnswer)

  if (globalAnswer === currentValue) { //after "=" was pressed...
    console.log('cyan-3a. after "=" pressed, just need one symbol added to currentValue')
  ...
  if (symbols.includes(currentValue)) { //if a symbol was already pressed...
    console.log('cyan-3b: currentValue and symbolString are both symbols!')
    currentValue = currentValue.replace(currentValue, symbolString)
    console.log('cyan-3b: replaced currentValue with ', symbolString)
    var newGlobalAnswer = globalAnswer.slice(0, -1) + currentValue
    console.log('cyan-4b: replaced lastChar in newGlobalAnswer with', currentValue)
    return storeAndReset(newGlobalAnswer, symbolString)
  } else { //otherwise, adding new symbol to currentValue...
    console.log('cyan-3c: adding new symbol to currentValue')
    currentValue = currentValue + symbolString //add symbol to end of currentValue...
    console.log('cyan-3c. new currentValue is: ', currentValue)
...
function changeBackground() {
  ...
  if (currentValue != "666") {
    ...
    console.log("A Legend was born in november ;)")
    $(document.body).addClass('background-1992');
  }
  if (currentValue != "1992") {
    $(document.body).removeClass('background-1992');
  }
  if (currentValue === "69") {
    console.log("no its not the sex-possition!")
    $(document.body).addClass('background-69');
  }
  if (currentValue != "69") {
    $(document.body).removeClass('background-69');
  }
}
